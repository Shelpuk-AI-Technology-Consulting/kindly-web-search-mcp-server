...
eighteen cases today, sixteen of which drive the script as a real process; the
other two are hermetic guards over the file itself (its imports, and that pytest
collects nothing from the directory). E3-1 delivered twelve of them; later steps
add flags to the script and cases beside them.

**That calibration deliberately does not run through the E3-4 harness** (§5.4a),
even though the harness generalises the same readiness, reaping and log-capture
shapes. This module is what makes the script a fixed point: driving it through a
second instrument would leave every failure here unable to say which of the two
was broken, and the harness's own calibration drives *this* script. It takes two
things from the harness and no more — `pid_is_alive` and `kill_pid`, forty lines
of platform probing with no opinion about how a test is structured.
...
| `--grandchild-depth N` | Make the descendant a chain of N; needs `--spawn-grandchild`. |
| `--chain-timeout SECONDS` | Ceiling on waiting for that chain to report itself. |
| `--stdout-bytes N` | N generated bytes on stdout, for a payload argv cannot carry. |
| `--grandchild-new-session` | Give that descendant its own session, as a browser has. |
| `--pid-file PATH` | Write the pids to PATH as JSON, **before** the readiness frame. |
...
expiry the script emits a `fixture.chain_incomplete` frame naming what it
expected and what it saw, reaps what it can name, and exits **70** — not 2,
which `argparse` already spends on an unknown flag, and a chain failure that read
as a bad command line would send the next reader to the command line. Announcing
anyway would hand a caller the half-built tree the wait exists to prevent;
hanging would surface as the caller's readiness timeout, which names neither.
`--grandchild-new-session` applies to the **first** generation only, which is
production's topology rather than a simplification: Chromium `setsid`s itself and
...
frame announces — which is the whole point of the flag, because a harness that
reaps from an announcement never walks anything.

`--stdout-bytes` exists because `--stdout` carries its text on the command line
...
"one level deep, and announced" limit is **closed** by `--grandchild-depth`:
generations two and upward announce nothing on the frame, so a harness given
only the root has to walk to them, and §5.4a's does. What remains is that the
chain is a *chain* — every generation has exactly one child — where a real
Chromium fans out into several renderers at one level. Nothing in the walk cares
about arity, and the case that would need a branching tree does not exist yet.
...
- Clean up in `finally`, keyed on **the PIDs this test spawned**. "No live
  ...
  **Amended by E3-4, because the literal rule cannot be met.** A descendant is
  not a pid the test spawned — it is a pid the *kernel* reported — and reaping
  only what the test spawned is precisely the defect production shipped: its
  tree is worker → Chromium → renderers, and only the first of those is
  announced. The invariant the harness holds is *"keyed on a pid this test
  spawned, plus the transitive descendants of that pid observed **while it was
  still alive**"*. That is still not a name scan, and the difference is not
  wording: the walk is rooted at a pid the caller owns and runs before that
  process dies, so a recycled pid cannot enter the set through it, whereas a
  name scan has no root at all.
...
| Helper | What it does |
|---|---|
| `spawned_child(argv, readiness_marker=…)` | Spawn, hand-shake, drain both pipes on threads, bound the block, capture on failure, reap the tree. |
| `wait_for_accepting_port(host, port, timeout=…)` | The socket form of the handshake, for an artefact with no pipe. |
| `reserve_ephemeral_port()` | A free port, for an artefact that needs one on its command line. |
| `descendants_of(pid)` | Every transitive descendant, by parentage. |
| `kill_process_tree(root)` | Descendants first, then the root, by pid only. |
| `pid_is_alive(pid)` / `wait_until_gone(pid)` | The platform probes, and a bounded wait on one. |

**The harness is told the root pid and discovers the rest.** That was the open
question this step was told to settle, and the answer follows from production
rather than from taste: `--spawn-grandchild` announces its descendant, so a
reaper built on the announcement is never made to *walk* anything — while the
tree it has to survive is worker → Chromium → renderers, where only the worker
announces. `tests/child_processes/worker_child.py` therefore gained
`--grandchild-depth N`, whose generations two and upward announce nothing.

**No group signal, anywhere, and that costs something.** Production's reaper
signals each descendant *group* before it signals pids, because Chromium leads a
group of its own and one call takes it with every renderer — including renderers
started after the walk. Correct there and forbidden here: a test process shares
its group with everything it spawns, so the same call written in a test reaches
the pytest session and the shell that started it. The cost is that a descendant
spawned after the walk is not reached by this harness. Stated rather than hidden,
...
reaped with them would measure a defect with itself, leaving a leaked browser
*and* a green teardown. The parse's one trap is duplicated too — the `comm` field
is unescaped and may hold a space and a `)`, so a whitespace split silently
yields a plausible small integer the caller then kills — and has its own case.

**The per-child deadline is a watchdog thread, timed from the handshake.** Every
other bound here is a caller-side wait, which cannot help when the block's own
body is what blocks; a thread can. Timing it from readiness rather than from the
spawn is what keeps it a bound on the *block*: from the spawn it would have to
...
**Failures carry the child's output as an exception note**, not as a wrapper. An
arbitrary exception cannot be rebuilt with a longer message —
`subprocess.TimeoutExpired` takes no such argument — so "attach the capture and
...
so `descendants_of` returns nothing there and the reap goes through
`taskkill /F /T`, which walks the tree inside the kill; its exit status is
...
fresh, uniquely-named, empty directory per call; a fourth one would be an unused
helper. It depends on an ordering worth naming: the harness's `finally` reap
completes *inside* the `with` block, before fixture teardown removes the
directory — on Windows a surviving child holding it open makes the removal fail.
And the *per-test timeout* stays per-module, because a fully-doubled test has no
child to bound: the two shapes already in the tree are an `asyncio.timeout`
around the awaited call (§5.2b) and a cap on the attempts a double will serve
(§5.4 above). "Shorter than the job timeout" has no referent yet — no job runs
this suite — so today it means shorter than a developer's patience, and E4-1 is
what gives the phrase a number.

`tests/harness/` gets **no** collection guard, unlike `tests/child_processes/`.
The guard there exists because a collected script would be *executed* at
collection time, writing frames onto the runner's stderr and possibly hanging.
...
**Result, `windows-latest`, 2026-09-06, merge candidate `7caab58`.**
...
This is the **seventh** run. Each one followed a review-driven change, which is
...
3. Repair the 3 stale loader tests. They assert command arguments and environment
   ...
   *"Protocol member WorkerProcess.returncode expected settable variable, got
   read-only attribute"*. E2-4 passes a real process to the annotated runner, so
   the attribute spelling would have made that step impossible. The cost, so it
   ...
   **Three mechanisms are needed, because none of them is sufficient alone.**
   ...
   2. A **static type-check job** (§10.3) is what catches signature drift, which
      ...
      **Two mechanisms, each earning its place by what it alone catches.**
      ...
      The block is guarded by `TYPE_CHECKING` deliberately: it reads members off
      the double, so at run time a missing member would raise `AttributeError`
      while the module is imported and abort collection — reporting one defect as
      a collection error rather than as the single conformance case that owns it.
      ...
      **Two configuration facts the harness depends on, both measured.** mypy's
      ...
      `mypy_path = "$MYPY_CONFIG_FILE_DIR/src"` (a relative path would resolve
...
diff-cover and packaging — into a clean Python 3.13 Linux virtualenv, then
...
`mypy` **was** in the pinned lane on the assumption that E2-1's negative
type-check fixture runs inside the ratcheted selection. **E2-1 settled it the
other way and mypy is now out of both this extra and the lockfile.** The harness
shells out to mypy, which §10.5's `subsystem` definition covers exactly ("a real
socket or child process"), so it carries that marker; this lane runs the `fast`
selection only and therefore never invokes mypy. Keeping it would install a
...
computed, not guessed: `.[ratchet]` was reinstalled into a clean Python 3.13
Linux virtualenv **constrained by the lockfile's own previous contents**, and the
frozen result differed by exactly those five entries. The constraint matters — an
unconstrained regeneration also absorbs whatever unrelated version drift has
accumulated since the file was last generated, and the reset rule above reserves
a moving baseline for a labelled `coverage-baseline-reset` change. The lockfile
header records the constrained command alongside the canonical one.
...
- **SerpBase's API key travelled in the URL and reached the MCP client in an
  ...
  **That boundary is now pinned by a case rather than recorded as prose**
  ...
  directions, seven classes): it is a version-scoped claim about a dependency
