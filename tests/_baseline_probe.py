...
process it spawns, and by :mod:`tests.test_ci_collection_floors`, which
loads it into a ``--collect-only`` child to read a CI job's selected count
without parsing pytest's summary line. It exists because the human-readable
