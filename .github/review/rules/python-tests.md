...
Eighteen tests exist to hold an invariant that nothing else enforces — **counted
against the table, not incremented.** This sentence has been wrong more often
than it has been right: it read "Fifteen" against sixteen rows, then "Sixteen"
against seventeen, because each pull request added one to what it found instead
of counting. Two branches then did it at once and met in a merge, which is how
the arithmetic shows itself — one arriving at seventeen rows, the other at
eighteen. Count the rows. A pull request
...
| Test | What it holds |
|---|---|
| `test_dependency_constraints.py` | the dependency bounds in `pyproject.toml` — the tooling extras **and** every runtime dependency, against the two tables in TEST_SUITE.md §10.2, in both directions. Beyond string agreement it holds two things a table-versus-table comparison cannot see, because a ceiling widened in the document and in `pyproject.toml` at once agrees with itself perfectly. First, each runtime ceiling **excludes every major above the declared floor's** — checked structurally (there must be a `<`/`<=` clause at or below the next major) *and* by probe, since the structural half alone admits `<=2` on the boundary and the probe half alone admits `!=2.*,!=3.*`. It is anchored to the **declared floor, not the lockfile pin**: §10.4 lets that pin move anywhere inside these bounds, so anchoring there would let a regenerated lockfile shift the reference major under a widened ceiling. Second, each floor still **admits** the version `requirements-ratchet.txt` pins — that direction genuinely needs the second file, and nothing else checks it, because the coverage lane installs `--no-deps` and pip never compares the two. It also holds that `[project].dependencies` contains exactly the documented set, each entry once — a duplicate is how an unbounded requirement gets past every other check, since the last one wins at resolve time. The escapes to watch: loosening a ceiling to make a resolve succeed, and raising a floor without regenerating the lockfile, which would leave the coverage lane measuring a version `pyproject.toml` forbids |
...
| `test_mcp_tool_schema_contract.py` | the JSON Schema clients actually bind to, read from `list_tools()` rather than from the source — parameter names, JSON types, required-ness and defaults against a golden, the exposed tool set, and the **absence** of an `outputSchema`, which is normative for this implementation rather than incidental. Normalization drops generated `title` keywords and description wording so an SDK minor release cannot fail it, and nothing else, so a key a future SDK *adds* does fail it. Two of its cases feed the normalizer a synthetic schema, because four of its five rules have no live input — the description sentinel, the `required` sort, the list recursion, and the level-awareness that keeps a parameter genuinely *named* `title` from being deleted from both sides and masking a real contract change. Only the `title` strip is exercised by the served payload. Deleting those cases, or widening the normalizer to drop more than the two cosmetic keywords, is weakening a guard. It also holds the concurrency ceiling the tool description states to the value the resolver **actually enforces**, probed at run time and compared against every prose copy — the description, the README, `.env.example` and `.github/review/rules/mcp-server.md`. The probe is what makes deletion detectable: a constant restated here would still agree with all four copies after the clamp was removed. Pinning fewer copies is the partial drift `test_provider_registry_consistency.py` exists to stop |
| `test_diagnostics_masking.py` | that credentials do not reach diagnostics output — **for the shapes its two rules cover, which is not all of them**. Masking is by variable *name* hint plus a userinfo redaction matching `://user:pass@`, so a variable named like nothing and valued like a schemeless `user:pw@host` is caught by neither. `SEARXNG_BASE_URL` is that shape whenever an operator mistypes it, and `test_a_schemeless_base_url_is_not_masked_today` **characterizes the gap rather than closing it** — it asserts today's disclosure, cites TEST_SUITE.md §14, and is meant to fail when the emit-boundary step lands. Deleting it to tidy up, or rewriting it to assert masking before that step exists, removes the only executable record that this stream is not fully held |
...
| `test_worker_child_fixture.py` | **two of its eighteen cases only.** The other sixteen calibrate a test instrument -- they drive the fixture child's flags against a real process, and a change to that script is meant to change them. The two that are load-bearing hold properties of the script rather than of any test: that `tests/child_processes/worker_child.py` imports **only** the standard library, and that pytest collects nothing from that directory. The first is what keeps the fixture startable at all -- the process runner hands its child a *complete* environment, and a path-invoked script never runs `tests/conftest.py`, so an import of the package would resolve or not depending on ambient path setup; the lifecycle step is still scheduled to edit that script, and nothing else enforces either property. The anti-flake harness step has now edited it — it added three flags and four cases, which is exactly the ordinary work this row exempts. The second has a cheap escape under a rename -- deleting the case -- and if it goes the script becomes collectable, which means executed at collection time, which means frames on the runner's stderr and possibly a hang. Weakening either is weakening a guard; editing the other sixteen alongside the script is ordinary work |
...
| `test_anti_flake_harness.py` | **two of its thirty-six cases only.** The other thirty-four calibrate a test instrument -- the shared harness under `tests/harness/`, which spawns children, hand-shakes with them, bounds them and reaps their process trees -- and a change to that harness is meant to change them. The two that are load-bearing are: that no module under `tests/harness/` imports the package under test, and that none of them names a **process-group** signalling primitive. The first is what keeps the instrument independent of its subject: the harness re-implements a `/proc` parent-pid parse and a descendant walk that the shipped process runner also has, and the step that tests *those* will reap its own trees with *these* -- a harness that imported production's would measure a defect with itself, leaving a leaked browser and a green teardown together. The second is the one that cannot be caught any other way. A test process shares its process group with everything it spawns, so `os.killpg(os.getpgid(child), ...)` written in a test kills the pytest session, its shell, and the report that would have named the problem; the assertion is made over syntax trees for that reason -- with `getpgrp` and `setsid` in its forbidden set as well as `killpg`, `setpgid` and `getpgid`, because `os.kill(-os.getpgrp(), SIGKILL)` reaches the caller's own group while naming none of the first three -- and eleven further cases drive the walk, the stat parse, the kill set and the reap lock against **injected** values with no process in existence. Adding a group signal here, trimming its forbidden set, or importing the runner to avoid duplicating forty lines, is weakening a guard; editing the other thirty-four alongside the harness is ordinary work |
| `test_worker_frame_contract.py` | **two of its forty-four cases only.** The other forty-two are ordinary L2 contract cases over the `KINDLY_DIAG` frame format (TEST_SUITE.md §4.3) — the codec's two directions, the seven stream shapes, the line ceiling — and editing them alongside the codec is ordinary work. The two that are load-bearing hold properties nothing else enforces. The first is that the frame marker has **one** definition: it sweeps every file under `src/` and `tests/` for the marker as a string or bytes literal in the *syntax tree*, and compares an exact per-file count against an allow-list naming four exempt files and five exempt sites: `scrape/universal_html.py` twice, for `_split_worker_diagnostics`, dead and awaiting removal; `tests/child_processes/worker_child.py`, a test instrument that must not import the package it calibrates; `tests/test_worker_child_fixture.py`, that instrument's calibration, which reads the frames as raw bytes and so cannot import production either; and `tests/test_worker_frame_contract.py` itself, whose single hand-written literal is **permanent by design** — once both sides of the protocol read one constant, every assertion computed through the codec moves with it, so that anchor is the only thing that kills a changed marker or changed JSON separators. Read the count off `MARKER_ALLOWANCE` rather than off this sentence, which has been wrong before. The count is exact rather than a floor because the `universal_html.py` pair is temporary and its departure is what the next reader needs to see; the other three exemptions are permanent, for the reasons given against each; the vocabulary is the marker **including its trailing space**, because `KINDLY_DIAGNOSTICS` shares the stem and would be swept in without it; and the sweep reads literals rather than text because several modules discuss the format in prose. Widening the allow-list to admit a new copy, rather than importing `FRAME_PREFIX`, is weakening a guard — the whole point of the step was that the format had four copies and a "keep in sync" comment holding two of them together. The second is that `nodriver_worker` defines no private line ceiling: that comment is what the step removed, and nothing else would notice it returning. Adding a copy of the marker, or a second ceiling constant, is weakening a guard |
| `test_corpus_policy.py` | the governance of the saved-HTML corpus under `tests/corpus/html/`, which in a **public** repository is a set of published documents. It holds the policy twice -- as a module constant and as the fenced JSON block in TEST_SUITE.md section 3.3 -- and compares them both ways, so a policy edit is a reviewed two-file edit. Because both copies are edited together, the obligations the design names are pinned a **third** time against literals in the module: the sanitation categories, the two tiers, each tier's required, either-or and forbidden field lists, and the patterned fields. Without that third anchor, dropping `capture_date` from the document and the constant in one pull request leaves every parametrized sweep smaller and the suite green -- that is the specific escape, and taking it is weakening a guard. One checker implements every rule and is driven two ways: once over the committed corpus, with each case filtering to a single rule; and once per rule over a synthetic corpus in `tmp_path` that breaks exactly that rule, asserting the **exact** violation list rather than a non-empty one. Sanitation rows carry positive *and* negative specimens, as lists, each driven individually: the positives prove a row is armed for every shape a leak takes -- `Set-Cookie:` is a response header, so a row armed only against it misses the `http-equiv` meta and the `document.cookie` assignment, which are how a cookie actually reaches committed HTML -- and the negatives pin measured false positives, an API documentation heading reading `Authorization: Bearer tokens` and the `logo@2x.png` retina filename. Deleting a specimen, collapsing the lists, or loosening a row to make a capture commit is weakening a guard; the honest move is to trim the capture. No snapshot *page* ships by design, so the committed-tree cases for the page- and sidecar-driven rules are vacuous until the first real snapshot lands, and only the synthetic cases carry those claims until then -- but the tier directory is **not** empty: `snapshots/README.md` is committed, and the size cap, sanitation sweep and encoding rules are measured against it today, each pinned by its own case. Carving the allowed filename out of those three sweeps is the escape a contributor takes when this table rejects a Markdown page they want, and it is weakening a guard |
| `test_provider_credential_disclosure.py` | that no search provider's credential reaches the **MCP client** in a tool error. Distinct from `test_diagnostics_masking.py`, which holds part of the diagnostics stream — **not all of it**: a schemeless `SEARXNG_BASE_URL` still reaches that stream unmasked, measured, and TEST_SUITE.md §14 carries it as an open gap owned elsewhere. This one holds what is *returned to the caller*, and for this server the caller is an LLM agent, so a credential here lands in a model provider's request and in a conversation transcript. Two layers, because neither proves the claim alone — the conversion in the router as a unit, and the payload a client is actually served, driven through the low-level `CallToolRequest` handler — not `mcp.call_tool`, which raises FastMCP's `ToolError` one layer above where the served text and `isError` are composed. The six-provider sweep is kept non-vacuous by a control asserting each row's credential was genuinely **in flight**: in the request URL for the two providers that carry it there, in a header for the four that do not. Deleting that control is the cheap escape, and it is the whole reason the sweep is evidence rather than eighteen assertions that a string is absent from a string that never held it — pointed at a header-authenticating provider, every one of them passes while proving nothing. Two further cases are load-bearing against a *widened* repair rather than a weakened one: a provider's own exception must pass through with its class and message intact, and an unconfigured server must still name the variables to set, so nobody satisfies the guard by catching everything and saying nothing. Loosening the URL-drop to a denylist of credential-shaped parameter names is weakening this guard: it fails open on the first provider that names its parameter something not on the list, which is the specific failure the drop exists to make impossible |
