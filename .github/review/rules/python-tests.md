...
Eighteen tests exist to hold an invariant that nothing else enforces. A pull request
...
| `test_ci_collection_floors.py` | that every `--min-selected` floor declared under `.github/workflows/` **equals** the number of tests its own selection collects, measured by collecting that selection in a child process. The floor is the only guard against a selector that silently matches too few tests -- one matching *nothing* exits 5 and fails the job, one matching a handful exits 0 and the job is green having run almost nothing -- and it went live 40 tests short, widening to 73 within the hour, because a count calibrated in one branch is invalidated by any other branch that merges first and nothing rechecked it at merge time. Equality rather than "at least" is the mechanism, not a detail: it forces every pull request that changes the collected count to edit one workflow line, so two branches that both change it collide in git instead of silently overwriting each other; relaxing it to `>=` restores the defect exactly. Four further things are load-bearing. The count comes from the probe plugin and **never** from pytest's summary line, which reports the pre-deselection total as soon as anything is deselected -- `857/860 tests collected (3 deselected)` -- and the first deselection arrives with the browser tests. The child **collects** and never runs: `test_baseline_failure_ledger.py`'s whole-suite child runs this module, so a child that ran its selection would spawn a whole-suite grandchild per floor. The set of floor-declaring workflow files is pinned as a literal, because a sweep that found nothing would satisfy every other case here by running none of them. And every check refuses rather than skips what it cannot resolve -- a command that is not a pytest invocation, an option the parser does not model, a floor in an unrecorded file, a `run:` body it cannot read, and **a child that exited non-zero** -- since a guard that quietly stops checking is the failure this module replaced. The last is the subtle one: a collection error still writes a probe file, so its smaller count reads as a stale floor and the message would say to write it into the workflow, shrinking the gate by exactly the tests the break removed. The child also runs without pytest's own steering variables, since `PYTEST_ADDOPTS` appends `--ignore` entries and would shrink the count the same way |
