...
against the table, not incremented.** Two branches each added a guard row and
each bumped this number by one, and the merge of them is how the arithmetic shows
itself: one arrived saying "Sixteen", the other "Seventeen", and the merged table
has eighteen rows. The sentence had also been wrong before either touched it —
"Fifteen" against sixteen rows. Count the rows; do not add one to what the
sentence already said. A pull request
